...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if the 'employees_elec' table exists, and create it if not
    $checkTableQuery = "SHOW TABLES LIKE 'employees_elec'";
    ...
    if (mysqli_num_rows($tableResult) == 0) {
        ...
        $createTableQuery = "CREATE TABLE employees_elec (
        ...
        if (mysqli_query($conn, $createTableQuery)) {
            echo 'Table "employees_elec" created successfully.';
        } else {
    ...
    $insertQuery = "INSERT INTO employees_elec (name, birthday, oldItem, newItem, position, sg, sg1, amount, amount1, office, employment, start)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        header('Location: ../pages/employee-elective.php');
        exit();
    } else {
